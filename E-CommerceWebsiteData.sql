# Q1) You are required to create tables for supplier,customer,category,product,supplier_pricing,order,rating to store the data for the Ecommerce
# Q2) Insert the data in the table created above
...
select count(t1.cus_gender) as No_Of_Customers, t1.cus_gender as customer_Gender 
from 
...
from `order` inner join supplier_pricing on 
...
# Q9) Create a stored procedure to display supplier id, name, rating and Type_of_Service. For Type_of_Service, If rating =5, print “Excellent Service”,If rating >4 print “Good Service”, If rating >2 print “Average Service” else print “Poor Service”.
 ...
 create procedure SelectSupplierDetails
as
select supplier.supp_id, supplier.supp_name ,t1.rat_ratstars ,t4.cat_name from supplier left join((select category.cat_name from category
      left join (select product.cat_id from product 
                       left join (select supplier_pricing.pro_id ,supplier_pricing.supp_id from supplier_pricing 
                                     left join (select rat_ratstars, `order`.pricing_id  from rating 
                                                   left join `order` on rating.ord_id = `order`.ord_id) as t1
									 on supplier_pricing.pricing_id=t1.pricing_id) as t2 
					   on product.pro_id=t2.pro_id) as t3 on category.cat_id=t3.cat_id) as t4) on t4.supp_id=supplier.supp_id
go;
